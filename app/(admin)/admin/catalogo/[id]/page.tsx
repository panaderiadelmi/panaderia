...
import { actualizarProducto } from '@/lib/actions/catalogo';
import { ALERGENOS, type Producto } from '@/lib/types';
...
export default async function EditarProductoPage({ params }: { params: { id: string } }) {
  ...
  const action = actualizarProducto.bind(null, p.id);
  ...
  return (
    <>
      ...
      <form action={action} style={{ maxWidth: '640px', display: 'flex', flexDirection: 'column', gap: '20px' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '16px' }}>
          ...
          <div>
            ...
            <input name="orden" type="number" min="0" className="form-input" defaultValue={p.orden} />
          </div>
        </div>
          ...
          <div style={{ display: 'flex', flexWrap: 'wrap', gap: '10px' }}>
            {ALERGENOS.map(a => (
              <label key={a} style={{ display: 'flex', alignItems: 'center', gap: '6px', fontSize: '0.85rem', cursor: 'pointer' }}>
                <input type="checkbox" name={`alergeno_${a}`} defaultChecked={p.alergenos?.includes(a)} style={{ accentColor: 'var(--color-primary)' }} />
                {ALERGENOS_ES[a]}
              </label>
            ))}
          </div>
          ...
          <div style={{ display: 'flex', flexWrap: 'wrap', gap: '20px' }}>
            {[
              ...
              <label key={key} style={{ display: 'flex', alignItems: 'center', gap: '8px', fontSize: '0.9rem', cursor: 'pointer' }}>
                <input type="checkbox" name={`mercado_${key}`} defaultChecked={p.mercados?.includes(key) ?? false} style={{ accentColor: 'var(--color-primary)' }} />
                {label}
              </label>
            ))}
          </div>
